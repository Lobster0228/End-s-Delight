    ...
    public void onSteppedOn(World world, BlockPos pos, BlockState state, Entity entity) {
        ...
        if (isLit && !entity.isFireImmune() && entity instanceof LivingEntity livingEntity) {
            if (!EnchantmentHelper.hasFrostWalker(livingEntity)) {
                entity.damage(ModDamageTypes.getSimpleDamageSource(world, ModDamageTypes.STOVE_BURN), 1.0F);
            }
        }
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        if (Boolean.TRUE.equals(state.get(CampfireBlock.LIT))) {
            ...
            if (random.nextInt(10) == 0) {
                world.playSound(dx, dy, dz, ModSounds.BLOCK_STOVE_CRACKLE.get(), SoundCategory.BLOCKS, 1.0F, 1.0F, false);
            }
