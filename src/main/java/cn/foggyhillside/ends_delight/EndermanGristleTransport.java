                ...
                if (flag) {
                    if (entity instanceof PlayerEntity && !((PlayerEntity) entity).isCreative()) {
                        if (entity.getHealth() < (entity.getMaxHealth() * 0.3F)) {
                            entity.damage(world.getDamageSources().create(EDModDamageTypes.ENDERMAN_GRISTLE_TELEPORT), entity.getHealth() * 1.5F);
                        } else {
                            entity.damage(world.getDamageSources().create(EDModDamageTypes.ENDERMAN_GRISTLE_TELEPORT), entity.getHealth() * damage);
                        }
                    }
                }
            }
